...
class IoContext_IncomingRequest final {
 public:
  ...
  SpanParent getRootUserTraceSpan() {
    KJ_IF_SOME(s, rootUserTraceSpan) {
      return s.addRef();
    }
 ...
 private:
  ...
  // Root user trace span for this request. Set during delivered() when a workerTracer is
  // present. The SpanParent's observer chain now holds a WeakRef<BaseTracer> (rather than a
  // strong kj::addRef), so storing it in IoOwn<SpanParent> in the AsyncContextFrame does not
  // extend the BaseTracer's lifetime past end-of-request.
  kj::Maybe<SpanParent> rootUserTraceSpan;
...
class IoContext final: public kj::Refcounted, private kj::TaskSet::ErrorHandler {
 public:
  ...
  // the given user trace span, or the current request's root user span. Stores the SpanParent
  // directly as IoOwn<SpanParent> in the AsyncContextFrame — the same pattern as
  // makeAsyncTraceScope for Jaeger tracing. This is safe because the SpanSubmitter now holds
  // a WeakRef<BaseTracer> rather than a strong ref, so IoOwn<SpanParent> on the delete queue
  // cannot extend the BaseTracer's lifetime past end-of-request.
