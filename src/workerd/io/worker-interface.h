...
class WorkerInterface: public kj::HttpService {
 public:
  ...
  struct AlarmResultWithException {
    bool retry = true;
  ...
  struct AlarmResult {
    ...
    AlarmResultWithException asOutcome() const {
      return {
  ...
  class AlarmFulfiller {
   public:
    AlarmFulfiller(kj::Own<kj::PromiseFulfiller<AlarmResultWithException>> fulfiller);
    ...
    void fulfill(const AlarmResultWithException& result);
    void reject(const kj::Exception& e);
    void cancel();

   private:
    kj::Maybe<kj::Own<kj::PromiseFulfiller<AlarmResultWithException>>> maybeFulfiller;
    kj::Maybe<kj::PromiseFulfiller<AlarmResultWithException>&> getFulfiller();
  };

  using ScheduleAlarmResult = kj::OneOf<AlarmResultWithException, AlarmFulfiller>;
