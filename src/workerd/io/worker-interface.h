...
class WorkerInterface: public kj::HttpService {
 public:
  ...
  struct AlarmResult {
    ...
    // Whether the alarm handler's failure was classified as user-generated by
    // isAlarmFailureUserError().  Carried from the handler's catch block to the
    // output-gate error handler so the latter cannot downgrade the classification.
    // Not serialised over RPC — only used within the sandbox process.
