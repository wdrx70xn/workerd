...
class IoChannelFactory {
 public:
  ...
  // Implementation for abortIsolate(). The default implementation shouldn't be
  // used anywhere. In workerd, the handler will abort the process. In the edge
  // runtime it will codemn and terminate the current isolate. Keeping the
  // default not-implemented helps ensure that workerd abort-the-process version
